...
uint8_t Init_Req[13][0x2a] = {{0x01, 0x03, 0x0c, 0x00},
                              ...
                              {0x12, 0x00, 0x00, 0xd1, 0x01, 0x0b, 0x00}, //1B local device name length and device name will be added at right
                              {0x12, 0x00, 0x00, 0xe1, 0x3d, 0x0b, 0x00, 0x03, 0xfc, 0x08, 0x00, 0xd1, 0x66, 0x98, 0x04, 0x00, 0x00, 0x00},
                              {0x12, 0x00, 0x00, 0xe1, 0x3d, 0x0b, 0x00, 0x03, 0xfc, 0x08, 0x00, 0xd1, 0x54, 0x98, 0x04, 0x00, 0x00, 0x00},
							  {0x14, 0x00, 0x00, 0xE1, 0x01, 0x0D, 0x00, 0x04, 0x02},
...
uint8_t Connect_Req[4][0x1f] = {{0x0f, 0x00, 0x00, 0xe1, 0x13, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
...
volatile TC_UART_Driver_State tc_driver_state = TC_BT_Before_Init;
volatile Init_Seq init_seq_current = Seq_HCI_Reset;
volatile Connect_Seq connect_seq_current = Seq_MNG_Connection_Accept1;
volatile uint8_t connect_waiting = 0; //bool

volatile uint16_t lrec = 0;								
TCU_Command* tcu_resp = (TCU_Command*) tc_uart_buff;
...
uint8_t array_equal(const volatile uint8_t* p1, const volatile uint8_t* p2, uint16_t length) //bool
{
	uint16_t i;
	for (i = 0; i < length; i++)
...
void modify_commands(void)
{
	...
	memcpy(Init_Req[Seq_MNG_Init2] + 10, tc_bt_conf.device_name, tc_bt_conf.device_name_length);
	Init_Req[Seq_MNG_Init2][0] = tc_bt_conf.device_name_length + 10;
	Init_Req[Seq_MNG_Init2][5] = tc_bt_conf.device_name_length + 3;
	Init_Req[Seq_MNG_Init2][9] = tc_bt_conf.device_name_length;
...
void step_init(void)
{
	...
	if (tc_driver_state == TC_BT_Before_Init) {
		...
		if (init_seq_current < 4) { //HCI Reply
			if (tc_uart_buff[0] != 0x04) return; //not HCI event
			if (tc_uart_buff[1] == 0x0f) { //HCI_Command_Status_Event
				if (array_equal(tc_uart_buff + 1 + 4, Init_Req[init_seq_current] + 1, 2)) //response of last command
					if (tc_uart_buff[1 + 2] != 0x00) { //not successful
			...
			if (tc_uart_buff[1 + 5] != 0x00) { //not successful
				tc_driver_state = TC_BT_Init_Failed;
				tc_bt_event_handler(TC_BT_Event_Init_Failed);
				return;
			}
			...
			{
				//TODO: check if a successful reply is received
			}
		init_seq_current++;
...
void step_connect(void)
{
	...
	if (tc_driver_state == TC_BT_Idle) {
		if (tcu_resp->service_id == 0xE1 && tcu_resp->op_code == 0x55) { //TCU_MNG_Connection_Request_Event			
			...
			if (tc_bt_conf.link_stored != 0x00) { //stored link exists
		...
		switch (connect_seq_current) {
			case Seq_MNG_Connection_Accept1:
				if (tcu_resp->service_id == 0xE1 && tcu_resp->op_code == 0x7D //TCU_MNG_SSP_Info_Event
				&&  tcu_resp->param[0] == 0x31 && tcu_resp->param[1] == 0x06)
					connect_seq_current++;
				else return;
				break;
			case Seq_MNG_HCI_Set1:
				if (tcu_resp->service_id == 0xE1 && tcu_resp->op_code == 0x7D //TCU_MNG_SSP_Info_Event
				&&  tcu_resp->param[0] == 0x33 && tcu_resp->param[1] == 0x0A)
					connect_seq_current++;
				else return;
				break;
			case Seq_MNG_HCI_Set2:
				if (! connect_waiting) {
					if (tcu_resp->service_id == 0xE1 && tcu_resp->op_code == 0x47) { //TCU_MNG_Connection_Status_Event
						TCU_MNG_Connection_Status_Event* st = (TCU_MNG_Connection_Status_Event*)(&(tcu_resp->param));
						if (st->connection_status != Connection_Status_Link_Key) return;
						
						memcpy(tc_bt_conf.valid_link.link_key, st->link_key, 16);
						tc_bt_conf.link_stored = 1;
						modify_commands();
						connect_waiting = 1;
					...
					if (tcu_resp->service_id == 0xE1 && tcu_resp->op_code == 0x55) { //TCU_MNG_Connection_Request_Event
						if (! array_equal(tcu_resp->param, tc_bt_conf.valid_link.remote_addr, 6))
							connect_seq_current = Seq_Reject;
						else
							connect_seq_current = Seq_MNG_Connection_Accept2;
						connect_waiting = 0;
				...
				break;
			case Seq_MNG_Connection_Accept2:
	...
	if (connect_seq_current == Seq_Reject){
		 //the first command in Connect_Req is Accept_Req, in which the first param is 00(accept) or 01(reject)
		((TCU_Command*)Connect_Req)->param[0] = 0x01;
		tc_uart_send_bytes(Connect_Req[0], Connect_Req[0][0]);
		((TCU_Command*)Connect_Req)->param[0] = 0x00; //change back
		tc_driver_state = TC_BT_Idle;
...
void spp_handler(void)
{
	...
	if (tcu_resp->service_id == 0xE5 && tcu_resp->op_code == 0x48) { //TCU_SPP_Data_Receive_Event
...
{
	if (init_seq_current >= Seq_HCI_Set_Mode) return;
...
extern void tc_bt_spp_send(uint8_t* bytes, uint16_t count)
{
	...
	uint8_t send[TC_BT_SPP_Single_Block_Max_Size + 3 + 6] = {0x00, 0x00, 0x00, 0xe5, 0x08};
...
extern void tc_uart_receive_handler(void)
{
	if (tc_driver_state == TC_BT_Init && init_seq_current < 4) { //HCI
		if (lrec == 0 && tc_uart_rec_count >= 1 + 2)
			lrec = (tc_uart_buff[1 + 2 - 1] & 0xff);
			*(((uint8_t*)(&lrec)) + 1) = 0x00; //to get rid of a bug
		if (tc_uart_rec_count == 1 + 2 + lrec) {
			step_init();
			lrec = 0; tc_uart_rec_count = 0; //clear buffer
		}
	} else {
		if (tc_uart_buff[0] == 0x00) tc_uart_rec_count = 0;
		if (lrec == 0 && tc_uart_rec_count >= 3)
			lrec = (((uint16_t)(tc_uart_buff[1])) << 8) + tc_uart_buff[0];
		if (tc_uart_rec_count == lrec) {
			if (tc_driver_state == TC_BT_Init)
				step_init();
			else if (tc_driver_state == TC_BT_Idle || tc_driver_state == TC_BT_Connecting)
				step_connect();
			else if (tc_driver_state == TC_BT_Connected)
				spp_handler();
			
			lrec = 0; tc_uart_rec_count = 0;
		}
	}
}
